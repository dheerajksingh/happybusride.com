...
async function main() {
  ...
  // 7. Routes, schedules, and trips — only created when cities exist in DB.
  //    Upload cities via Admin → Cache Management before running seed to get demo routes.
  ...
  if (!delhi || !mumbai || !bengaluru || !chennai || !hyderabad || !pune || !jaipur || !ahmedabad) {
    console.log("⚠️  Demo routes/schedules/trips skipped — required cities not in DB.");
    console.log("   Upload a cities CSV via Admin → Cache Management then re-run seed.");
  } else {
    // Routes
    ...
    // Trips — generate for next 30 days
