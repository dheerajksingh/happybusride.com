...
export async function POST(req: NextRequest) {
  ...
  const config = await prisma.cabPricingConfig.findUnique({ where: { id: configId } });
  ...
  const prompt = `You are a pricing function generator for a local cab service in a bus ticketing platform.
...
1. Takes two parameters: distanceKm (number), vehicleType (string — one of "Sedan", "SUV", "Hatchback", "Auto")
...
Return ONLY the JavaScript function body code, no explanation, no markdown, no code fences.`;
  ...
  try {
    ...
    try {
      ...
      const testResult = testFn(10, "Sedan");
    ...
    const updated = await prisma.cabPricingConfig.update({
      where: { id: configId },
      data: { generatedFn: fnBody, generatedAt: new Date(), isActive: true },
    });

    // eslint-disable-next-line no-new-func
    const previewPrice = new Function("distanceKm", "vehicleType", fnBody)(10, "Sedan");
    return NextResponse.json({ config: updated, preview: `Test (10km, Sedan): ₹${previewPrice}` });
  } catch (err: any) {
