...
export async function POST(req: NextRequest) {
  ...
  const config = await prisma.shuttlePricingConfig.findUnique({ where: { id: configId } });
  ...
  const prompt = `You are a pricing function generator for a local shuttle service in a bus ticketing platform.
...
1. Takes two parameters: distanceKm (number), vehicleType (string — one of "SEATER_6", "SEATER_8", "SEATER_10")
...
The function body will be used like this:
  const fn = new Function('distanceKm', 'vehicleType', YOUR_FUNCTION_BODY);
...
Return ONLY the JavaScript function body code, no explanation, no markdown, no code fences.`;
  ...
  try {
    ...
    try {
      ...
      const testFn = new Function("distanceKm", "vehicleType", fnBody);
      const testResult = testFn(10, "SEATER_8");
    ...
    const updated = await prisma.shuttlePricingConfig.update({
      where: { id: configId },
      data: { generatedFn: fnBody, generatedAt: new Date(), isActive: true },
    });

    // eslint-disable-next-line no-new-func
    const previewPrice = new Function("distanceKm", "vehicleType", fnBody)(10, "SEATER_8");
    return NextResponse.json({ config: updated, preview: `Test (10km, SEATER_8): ₹${previewPrice}` });
  } catch (err: any) {
