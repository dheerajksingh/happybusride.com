...
const PLACEHOLDER_PRICING = `# Extra luggage pricing (weight over 20kg limit)
Price, ExcessKg, DistanceKm
50,    5,        100
100,   10,       100
150,   15,       100
75,    5,        200
...
export default function AdminExtraLuggagePricingPage() {
  ...
  const [testExcess, setTestExcess] = useState(5);
  const [testDist, setTestDist] = useState(100);
  const [testResult, setTestResult] = useState<number | null>(null);

  useEffect(() => {
    fetch("/api/admin/pricing/extra-luggage").then(r => r.json()).then(d => {
      if (d.config) {
  ...
  async function savePricingText() {
    setSaving(true);
    const res = await fetch("/api/admin/pricing/extra-luggage", {
  ...
  async function generateFunction() {
    ...
    const res = await fetch("/api/admin/pricing/extra-luggage/generate", {
      method: "POST", headers: { "Content-Type": "application/json" },
  ...
  function runTest() {
    ...
    try {
      ...
      const fn = new Function("excessWeightKg", "distanceKm", generatedFn);
      setTestResult(fn(testExcess, testDist));
    } catch {
      setTestResult(null);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">Extra Luggage Pricing</h1>
        <p className="mt-1 text-sm text-gray-400">
          Define pricing rules for luggage over the 20kg free limit. Claude generates the price calculator.
        </p>
      </div>
      ...
      <div className="rounded-xl bg-gray-800 border border-gray-700 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Format: Price, ExcessKg, DistanceKm</p>
          </div>
          {isActive && (
      ...
      {generatedFn && (
        <div className="rounded-xl bg-gray-800 border border-gray-700 p-6">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              <label className={labelCls}>Excess Weight (kg over 20kg)</label>
              <input type="number" className={numCls} value={testExcess}
                onChange={e => setTestExcess(Number(e.target.value))} />
            </div>
            <div>
