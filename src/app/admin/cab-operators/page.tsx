...
export default function AdminCabOperatorsPage() {
  const [operators, setOperators] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [updating, setUpdating] = useState<string | null>(null);

  useEffect(() => {
    fetch("/api/admin/cab-operators").then(r => r.json()).then(d => {
  ...
  async function updateStatus(id: string, status: string) {
    setUpdating(id);
    const res = await fetch(`/api/admin/cab-operators/${id}/status`, {
      method: "PATCH",
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">Cab Operators</h1>
        <p className="mt-1 text-sm text-gray-400">Manage cab operator registrations and status.</p>
      </div>
      ...
      <div className="rounded-xl border border-gray-700 bg-gray-800 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400">No cab operators registered yet.</div>
        ) : (
              ...
              {operators.map(op => (
                <tr key={op.id} className="hover:bg-gray-700/30">
                  <td className="px-5 py-4">
                    ...
                    <div className="text-xs text-gray-400">{op.user?.email ?? op.email}</div>
                  </td>
                  ...
                  <td className="px-5 py-4 text-gray-300">
                    <div>{op.vehicleType ?? "—"}</div>
                    <div className="text-xs text-gray-500">{op.vehicleReg ?? ""}</div>
                  </td>
