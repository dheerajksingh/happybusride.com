...
export default function ShuttleRegisterPage() {
  const router = useRouter();
  const [selectedCity, setSelectedCity] = useState<City | null>(null);
  const [form, setForm] = useState({
    fullName: "", address: "", phone: "", whatsapp: "",
    email: "", password: "", regNo: "", driverName: "", driverPhone: "",
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/shuttle/register", {
      method: "POST",
    ...
    router.push("/shuttle/login?registered=1");
  }

  const inputCls = "w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-teal-500 focus:outline-none";
  const labelCls = "mb-1 block text-sm font-medium text-gray-700";

  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-teal-50 to-cyan-100 px-4 py-10">
      <div className="w-full max-w-lg rounded-2xl bg-white p-8 shadow-xl">
        <div className="mb-6 text-center">
          <Link href="/" className="text-2xl font-black text-teal-600">HappyBusRide</Link>
          <div className="mt-1 text-lg font-bold text-gray-900">Shuttle Operator Registration</div>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className={inputCls} value={form.regNo} onChange={e => update("regNo", e.target.value)} />
            </div>
            <div>
        ...
        <p className="mt-4 text-center text-sm text-gray-500">
          Already registered? <Link href="/shuttle/login" className="font-semibold text-teal-600 hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
